...
function Hero() {
  ...
  useEffect(() => {
    ...
    // 2. TÍTULO - Stagger letters respetando color dorado
    const titleElement = titleRef.current;
    
    // Guardar las líneas originales con su HTML
    const line1 = "FAVELA'S";           // Blanco
    const line2 = "BARBER";             // Dorado
    
    // Limpiar título
    titleElement.innerHTML = "";
    
    // Línea 1 - FAVELA'S (BLANCO)
    const line1Div = document.createElement("div");
    line1Div.style.display = "block";
    line1Div.style.color = "#ffffff";
    ...
    titleElement.appendChild(line1Div);
    
    // Línea 2 - BARBER (DORADO)
    const line2Div = document.createElement("div");
    line2Div.style.display = "block";
    line2Div.style.color = "#F59E0B";
    ...
    titleElement.appendChild(line2Div);

    // Animar todas las letras con stagger
    tl.fromTo(titleElement.querySelectorAll("span"),
      { opacity: 0, y: 30, filter: "blur(4px)" },
      { opacity: 1, y: 0, filter: "blur(0px)", duration: 0.5, stagger: 0.03, ease: "back.out(2)" }
    );
